...
import Data.Classes (SubType(..))
import qualified Data.Classes as C
...
instance C.Boolean BitVector where
  true    = BV 1 1 0
  false   = BV 1 0 0
  isTrue  = C.isTrue  . flip getBit 0
  isFalse = C.isFalse . flip getBit 0
  not     = bv_not
  (&&)    = bv_and
  (||)    = bv_or
  xor     = bv_xor
...
bv_fold_xor v@(BV w a b)
  = if b == 0 then inj (f a False) else U
  where
    f 0 r = r
    f n r = f (n .&. (n - 1)) (Prelude.not r)
...
bv_fold_xnor v@(BV w a b)
  = if b == 0 then inj (a == 0 || a == 2^w-1)
    else if or xs && not (and xs) then F
    else U
  where
    -- get all known bits
    xs :: [Bool]
    xs = mapMaybe prj (toBits v)
...
instance C.Eq BitVector Bit where
  (==) = eq
  (/=) = neq

instance C.Eq BitVector Bool where
  (==) x y = eq_xz x y
  (/=) x y = not (eq_xz x y)

instance C.Compare BitVector where
  min = error "TODO min"
  max = error "TODO max"

instance C.Ord BitVector Bit where
  (>)  = gt
  (<)  = lt
  (>=) = gte
  (<=) = lte

eq :: BitVector -> BitVector -> Bit
eq (BV w0 a0 b0) (BV w1 a1 b1)
  -- if all bits are known, then a0 == a1
  -- else if there are any known bits that are unequal, then False
  -- else Unknown.
  = if b0 == 0 && b1 == 0 then inj (a0 == a1)
    else if (maskWidth w (complement b0 .&. complement b1 .&. (xor a0 a1))) /= 0
         then F
    else U
  where
    w = max w0 w1

neq :: BitVector -> BitVector -> Bit
neq (BV w0 a0 b0) (BV w1 a1 b1)
  -- if all bits are known, then a0 /= a1
...
eq_xz :: BitVector -> BitVector -> Bool
eq_xz (BV _w0 a0 b0) (BV _w1 a1 b1)
  = a0 == a1 && b0 == b1

lt :: BitVector -> BitVector -> Bit
lt v0@(BV w0 a0 b0) v1@(BV w1 a1 b1)
  = if b0 == 0 && b1 == 0 then inj (a0 < a1)
    else f (w-1)
  where
    w = max w0 w1
...
lte :: BitVector -> BitVector -> Bit
lte v0@(BV w0 a0 b0) v1@(BV w1 a1 b1)
  = if b0 == 0 && b1 == 0 then inj (a0 <= a1)
    else f (w-1)
  where
    w = max w0 w1
...
gt :: BitVector -> BitVector -> Bit
gt v0@(BV w0 a0 b0) v1@(BV w1 a1 b1)
  = if b0 == 0 && b1 == 0 then inj (a0 > a1)
    else f (w-1)
  where
    w = max w0 w1
...
gte :: BitVector -> BitVector -> Bit
gte v0@(BV w0 a0 b0) v1@(BV w1 a1 b1)
  = if b0 == 0 && b1 == 0 then inj (a0 >= a1)
    else f (w-1)
  where
    w = max w0 w1
